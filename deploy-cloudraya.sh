...
docker run -d --restart unless-stopped --platform linux/amd64 -p $APP_SERVER_PORT:$APP_SERVER_PORT --env-file .env --name $CONTAINER_NAME  $CLOUDRAYA_REGISTRY_URL:$CLOUDRAYA_REGISTRY_USERNAME:$IMAGE_TAG
exit
ENDSSH
